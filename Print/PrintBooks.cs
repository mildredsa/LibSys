...
namespace LibSys
{
    ...
    {
        string connectionString = "Data Source=LAPTOP-M3IQH77K\\MSASERVER;Initial Catalog=DBS;Integrated Security=True";
        public PrintBooks()
        {
            InitializeComponent();
        }
        ...
        private void PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                SqlCommand command = new SqlCommand("SELECT AccessionNumber, Title, Author FROM Books WHERE Status <> 'Lost'", con);
                SqlDataReader reader = command.ExecuteReader();

                // Add the header
                string headerText = "Books List Report";
                ...
                SqlCommand countBooks = new SqlCommand("SELECT COUNT(*) AS total_books FROM books WHERE status <> 'Lost'", con);
                int totalBooks = (int)countBooks.ExecuteScalar();

                // Add the footer
                Font footerFont = new Font("Arial", 10, FontStyle.Bold);
                string footerText = "Total Books: " + totalBooks.ToString();
        ...
        private void PrintBooks_Load(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("SELECT AccessionNumber, Title, Author FROM Books WHERE Status <> 'Lost'", con);
